...
**Research inputs:** Internal discovery · BU stakeholder interviews · CS Leaders Roundtable (June 4, 2025, hosted by Cowrkr)
...
2. [Discovery Methodology](#2-discovery-methodology)
   - [2.5 External Research Validation — CS Leaders Roundtable](#25-external-research-validation--cs-leaders-roundtable-june-4-2025)
...
## 2.5 External Research Validation — CS Leaders Roundtable (June 4, 2025)

*Source: CS Leaders Roundtable Summary, hosted by Cowrkr. A cross-industry group of
Customer Success leaders convened to discuss where renewals and expansion opportunities
slip through before they appear in the forecast.*

This session was attended as part of the discovery process for this initiative. The
six themes that emerged independently validate — and in several cases sharpen — the
design decisions in this BRD. Each theme maps directly to a component or principle
in the Realized ARR framework.

---

### Theme 01 → Validates TTFV and the Sustained Usage Score

> *"Between 70% and 90% of churn is seeded in the first two weeks of onboarding.
> The problem is not that teams do not know — it is that they rationalize early
> friction as normal, and by the time renewal comes around, the pattern is already set."*

Three failure moments were identified: onboarding friction never fully resolved,
first value assumed rather than confirmed, and signals seen but not acted on with
a clear playbook.

**BRD connection:** This is the empirical basis for the Time-to-First-Value (TTFV)
Decay Factor (Section 6.2, Signal 1). The 45-day TTFV penalty threshold in the
Realized ARR formula is not arbitrary — it reflects the industry-validated finding
that the first two weeks of onboarding set the churn trajectory. An account where
first compute credit consumption does not occur within 45 days of contract start
has already entered the at-risk pattern.

The Sustained Usage Score's 30-day minimum threshold for a "healthy month" directly
addresses the "onboarding friction rationalized as normal" failure. Without a
sustained usage component, a deployment in Month 1 followed by no usage in Month 2
still shows as a successfully onboarded account. The SUS component catches this by
measuring consistency across the full trailing window, not just the first event.

---

### Theme 02 → Validates the Composite PRS Approach and Adds a Missing Signal

> *"Usage data is a lagging indicator. Sentiment and engagement shift first.
> The customer-side sponsor and user handoff is a signal most teams completely miss.
> Tracking variance against the customer's own stated usage goals gives a much more
> honest picture than internal benchmarks."*

**BRD connection:** This independently validates the four-component design of PRS.
The Technical Health Score and Expansion Momentum components capture the leading
indicators (health trend, engagement trajectory) that usage data alone misses.
The point about internal benchmarks directly validates the Credit Burn Velocity (CBV)
signal — measuring actual burn against the customer's own committed baseline (their
included_monthly_compute_credits) rather than against a generic industry average.

**New signal surfaced — Customer Sponsor/User Handoff:**
The roundtable identified sponsor and user handoff as a signal most CS teams
completely miss. In the PANW context this maps to: when the security architect or
CISO who championed the purchase leaves, there is typically no corresponding
leadership contact update in the CRM. The account enters an engagement gap — the
CSM is still calling the old sponsor's number while the platform sits unmaintained.
The Zero-Telemetry Days (ZTD) alert catches the operational consequence (usage
stops). A future enhancement is to track CSM-contact-title changes in the CRM as
a leading indicator that fires *before* ZTD — the sponsor departure is detectable
30–60 days before usage drops.

---

### Theme 03 → Validates the Shared Commercial Denominator Design Principle

> *"Sales closes and moves on. CS inherits adoption. Support logs tickets that nobody
> connects back to churn risk. An increase in support tickets can look like revenue
> for one team while signaling a product failure for another.
> The fix is not better tools — it is a shared metric that ties every function back
> to the same commercial outcome. If everything is tied to a dollar value, people
> will own it."*

**BRD connection:** This is the external validation of the entire North Star metric
rationale. The roundtable articulates exactly the problem Realized ARR solves — not
at PANW specifically, but as an industry pattern across CS organizations. The
"support tickets as revenue for one team, product failure signal for another" example
is the Cost-to-Serve Index problem (Section 3.2, Pain Point 3) stated by practitioners
in a different industry.

The design principle "if everything is tied to a dollar value, people will own it"
is why Realized ARR is expressed in dollars, not as a ratio or score. A VP of
Customer Success presenting to the board says "$665K unrealized ARR in the banking
segment" — not "PRS is 0.335." The dollar framing creates ownership. The score
creates reporting.

---

### Theme 04 → Refines the Compensation Model Design

> *"Hard dollar expansion quotas on CSMs led to lead hoarding and conflict with sales.
> Shifting to an NRR model encouraged CSMs to pass leads to sales — everyone won.
> A tiered 70/30 model tied to GRR and NRR percentages, with multipliers and a cap,
> was one approach that worked well. Rules of engagement between CS and sales on
> what counts as a renewal versus a new sale must be crystal clear."*

**BRD connection:** The compensation model in Section 9.4 is validated and sharpened
by this finding. The roundtable confirms that hard expansion quotas on CSMs create
the exact conflict they are meant to prevent — CSMs compete with sales rather than
collaborating. The recommended 40/35/25 structure (base commission / eARR milestone
/ expansion conversion) avoids this by making the CSM's expansion income dependent
on a milestone that *requires* sales collaboration, not one that creates a parallel
sales motion.

The 70/30 GRR/NRR tiered model with a cap is the industry-tested version of the
same design principle. For PANW's context, this translates as: the CSM's base
performance is measured on Gross Revenue Retention (not churning the account) while
the multiplier is tied to Realized ARR efficiency improvement — the PRS movement
from below 0.60 to above 0.80. This is a refinement worth testing against the
current 40/35/25 model.

**New compensation model option surfaced:**

| Model | CSM Incentive Structure | PANW Applicability |
|---|---|---|
| **Hard expansion quota** | Fixed $ target on new ARR | ❌ Creates CS/Sales conflict |
| **Current recommendation (40/35/25)** | Base + eARR milestone + expansion conversion | ✅ Recommended |
| **70/30 GRR/NRR tiered** | 70% on retention, 30% on NRR improvement with multiplier | ✅ Viable alternative — test in one region |
| **Pure NRR model** | All comp tied to net retention outcome | ⚠️ Aligns collaboration but removes early-stage incentives for deployment speed |

---

### Theme 05 → Validates the "Outcomes Not Activity" Design Principle

> *"Health scores must be built on defined customer outcomes, not just activity metrics.
> Telling customers why they are successful — and making their internal teams look good
> — drives product love more than any feature.
> The open question: how accurate will AI-generated health signals be versus the human
> CSM gut feel? Nobody has a definitive answer yet."*

**BRD connection:** This validates Design Principle 2 in Section 5.2: "The metric
must encode business consequences, not just behavior." The roundtable confirms that
activity metrics (logins, feature usage, session counts) are insufficient — the
health score must be grounded in whether the customer is achieving security outcomes.
The Technical Health Score and the Deployment vs. Efficacy Illusion signal are the
PANW-specific embodiments of this principle.

The "AI vs. human CSM gut feel" question is directly relevant to Section 10. The
honest answer from this BRD's perspective: AI-generated health signals are more
consistent and more scalable than CSM gut feel, but they are blind to the signals
CSMs carry — relationship quality, executive sentiment, organizational politics,
sponsor change. The right architecture is AI for breadth (flag every account at
scale, every week) and human for depth (CSM investigates and adds context to the
flagged accounts). Neither replaces the other.

---

### Theme 06 → Validates the Entire ARR Transition Context

> *"Services and software are both moving from fixed cost to outcome-based pricing.
> Companies offering flexible, value-first pricing — pay when you hit your first
> outcome — will get in the door fastest."*

**BRD connection:** This is the macro trend that makes the TCV-to-ARR transition at
PANW not just a pricing change but a structural shift in how enterprise software
value is defined and measured. The roundtable confirms that PANW's transition is
not an isolated decision — it reflects an industry-wide movement where customers
are increasingly unwilling to pay for committed capacity they may not use. The
Realized ARR metric is PANW's internal response to the same pressure: GCS must earn
the ARR by delivering outcomes, not just by holding the contract.

The "pay when you hit your first outcome" pricing model maps directly to the TTFV
metric and the eARR Velocity KPI. If the external market is moving toward
outcome-based pricing, GCS's internal operating model must be ready to measure
and deliver those outcomes at the account level before the pricing model forces it.

---

### Roundtable-to-BRD Signal Map

| Roundtable Theme | Key Insight | BRD Component Validated or Added |
|---|---|---|
| Theme 01: Renewals slip early | 70–90% churn seeded in first 2 weeks | TTFV Decay Factor (Section 6.2) · Sustained Usage Score (Section 9.2) |
| Theme 02: Plural signals | Usage is lagging; sponsor handoff is missed | CBV vs. customer baseline · New: Sponsor Departure Signal (Phase 2) |
| Theme 03: Shared denominator | Dollar-tied metrics create ownership | Realized ARR in $ not score · Cross-functional alignment design (Section 5) |
| Theme 04: Compensation design | NRR model aligns CS/Sales; hard quotas create conflict | 40/35/25 model validated · 70/30 GRR/NRR as alternative (Section 9.4) |
| Theme 05: Health score trust | Build on outcomes, not activity; AI vs. gut feel open question | Design Principle 2 (Section 5.2) · AI + human architecture (Section 10) |
| Theme 06: Outcome-based future | Per-seat under pressure; pay on first outcome wins | ARR transition context (Section 1) · TTFV and eARR Velocity (Section 6) |
...
*Validated by the CS Leaders Roundtable (Theme 04): Hard dollar expansion quotas
on CSMs caused lead hoarding and CS/Sales conflict. The NRR model — where CSMs are
paid on retention and growth together — encourages CSMs to pass expansion leads to
sales rather than compete with them. A tiered 70/30 GRR/NRR model with multipliers
and a cap was cited as one approach that resolved the conflict in practice.*

**Primary Recommendation — 40/35/25 Model:**

- **40% Base Commission:** On contract execution based on committed cARR. Maintains
  sales momentum and cash flow visibility.
- **35% eARR Milestone Release:** Account reaching 80% Realized ARR efficiency
  within 180 days post-signature. Requires CS/Sales/PS collaboration to achieve —
  by design. No rep can hit this alone, which eliminates the CS/Sales lead-hoarding
  dynamic the roundtable identified.
- **25% Expansion Revenue:** Overage conversion to new committed ARR. Expansion
  Cannibalization Rate (Signal 3, Section 6.2) is the control that prevents ghost
  revenue from triggering unearned commission.

**Alternative Recommendation — 70/30 GRR/NRR Tiered Model (test in one region):**

| Layer | What It Measures | Weight | Multiplier |
|---|---|---|---|
| Gross Revenue Retention (GRR) | Did the account renew at committed ARR? | 70% | 1.0× base |
| Net Revenue Retention (NRR) improvement | Did Realized ARR efficiency improve vs. prior period? | 30% | 1.0–1.5× based on PRS band movement |
| Cap | Maximum payout ceiling | — | 1.5× total |

Rationale for testing this alternative: the 70/30 model is already in use at peer
CS organizations and has a track record of resolving the CS/Sales alignment problem
without requiring the complex eARR milestone calculation infrastructure that the
40/35/25 model depends on. Running it in parallel in one region would generate
comparative behavioral data within two quarters.

**Rules of engagement — explicitly required by the roundtable:**
The boundary between a CSM-sourced renewal and a sales-sourced new sale must be
defined before any compensation model goes live. Ambiguity here is where the conflict
resurfaces regardless of model design. Recommended definition: any expansion of an
existing contract within the current contract term is a CSM-owned renewal motion;
any new product line not in the current contract is a sales-owned new sale.
Both parties earn on the outcome via their respective tracks.
...
The CS Leaders Roundtable surfaced this gap precisely:

> *"The open question: how accurate will AI-generated health signals be versus the
> human CSM gut feel? Nobody has a definitive answer yet."*

The honest answer from this framework's perspective: AI-generated health signals are
more consistent and more scalable than CSM gut feel, but they are blind to the
signals CSMs carry in their heads — relationship quality, executive sentiment,
organizational politics, sponsor departure before it shows in the data. The right
architecture is not AI replacing human judgment. It is AI providing breadth (flag
every account at portfolio scale, every week) while the human provides depth (CSM
investigates and adds context to the specific flagged accounts). Neither works alone.
Both together is the architecture.
...
| CVI > 2.5 SD above baseline with no open P1/P2 tickets | Likely automated script errors spinning phantom workloads | Engineering investigation before false overage invoice generated |
| TTFV > 45 days across 20%+ of new accounts in a region | Onboarding process failure — systemic change-control barrier in that market | Regional PS capacity review, onboarding playbook update |
...
| ZTD > 14 days on accounts with CSM last-touch > 21 days | Silent churn in progress — account abandoned | Automated CSM escalation with AI narrative generated |
| Commitment Exhaustion Trajectory projects exhaustion > 30 days before contract end | Customer will go dark before renewal — invoice shock imminent | Immediate credit renegotiation, expansion offer |
| **CRM contact-title change on primary sponsor** *(Phase 2 signal)* | Customer-side champion departed — platform may go unmaintained before usage drops show up in ZTD | CSM outreach within 72 hours to re-establish executive sponsor relationship before the account goes dark |
...
*Version 3.0 — built using a spec-driven AI development methodology.*  
*All requirements defined in Markdown before any code was written.*  
*External research: CS Leaders Roundtable, June 4 2025, hosted by Cowrkr.*
